...
j2_env = Environment(loader=FileSystemLoader(path.join(dir_path, 'templates')),
	trim_blocks=True)
...
def make_deploy_config():
	...
	# for example, if the sampling period is 100000 us (10 Hz), then the
	# converted speed value will be 3 (3 m/0.1s = 30 m/s)
	...
	deploy_config['flight_minimal_altitude'] = 0
	deploy_config['flight_maximal_alitude'] = 4000
